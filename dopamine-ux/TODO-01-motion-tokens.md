...
Repo: `~/src/utils` (git repo, has CI + the file-length gate wired).

**Create** `~/src/utils/unified-design-system/motion.md` — the prose freeze.

**Do NOT append to `~/src/utils/unified-design-system/tokens.md`.** It is 260 lines
...
| Stack | File | Form |
|---|---|---|
| Flutter/Dart | `~/src/utils/design_system/lib/src/tokens.dart` | `class AppDuration` / `class AppCurve` |
| web/React | `~/src/utils/web_ui/src/tokens.css` | `--duration-*`, `--ease-*` custom props |
| Python/Tkinter | `~/src/utils/gatelock/gatelock/_window.py` (`class LockConfig`) | int ms fields — **only if meaningful**; see `must not` |
...
- Keep every file at or under **250 lines** (`~/src/utils/file_length/config.py`,
  `MAX_LINES = 250`). `.md` and `.py` are capped; `.css` is not.
- **Cut tags at the end** — this is the step that makes the work reach consumers:
  - `design_system-v0.2.0` (consumed by `~/src/todo`, currently on `v0.1.0`)
  - `web_ui-v0.3.2` (consumed by `~/src/dufs-cloud/web`, currently on `v0.3.1`)
...
- optional: a `prefers-reduced-motion` helper in
  `~/src/utils/design_system/lib/src/` and/or `web_ui/src/` so consumers get the
...
1. `~/src/utils/unified-design-system/motion.md` exists, is ≤250 lines, and defines
   duration, easing, haptic and sound vocabularies with a one-line rationale each.
2. `python3 ~/src/utils/file_length/check.py` (or
   `~/src/utils/scripts/check_file_length.sh --all`) reports **no new** violations.
...
3. `python3 ~/src/utils/unified-design-system/scripts/palette_check.py` still exits 0.
4. Both tags exist: `git -C ~/src/utils tag | grep -E 'design_system-v0.2.0|web_ui-v0.3.2'`
   prints two lines.
...
- `~/src/utils/unified-design-system/tokens.md` — the existing freeze; match its
  table format and tone exactly. Note its length problem before editing.
- `~/src/utils/unified-design-system/screen-size.md` — the precedent for a sibling
...
- `~/src/utils/web_ui/src/tokens.css` — the full custom-prop list.
- `~/src/utils/design_system/lib/src/feedback.dart` — `showToast`/`showError` and
...
- `~/src/utils/unified-design-system/scripts/palette_check.py` — read the four
...
- `~/src/utils/gatelock/` is a package inside the `~/src/utils` monorepo, not a
  standalone repo. It is consumed as a pinned git dependency.
- Only `~/src/todo` depends on `design_system`; diet-guard, workout_app and
